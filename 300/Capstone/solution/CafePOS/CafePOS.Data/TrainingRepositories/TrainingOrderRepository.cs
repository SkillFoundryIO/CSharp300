...
namespace CafePOS.Data.TrainingRepositories
{
    public class TrainingOrderRepository : IOrderRepository
    {
        public void AddItemsToOrder(List<ItemToAdd> orderItems)
        {
            ...
            {
                var eachPrice = FakeDb.ItemPrices
        ...
        {
            var item = FakeDb.Items
                .Where(i => i.CategoryID == categoryId)
                .ToList();

            var itemPrice = FakeDb.ItemPrices
        ...
        {
            var orderItemList = FakeDb.OrderItems
            ...
            foreach (var order in orderItemList)
            {
                ...
                var itemId = FakeDb.ItemPrices
                    .FirstOrDefault(ip => ip.ItemPriceID == itemPriceId).ItemID;
